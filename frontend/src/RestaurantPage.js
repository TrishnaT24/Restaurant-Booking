...
const RestaurantPage = () => {
  ...
  const fetchRestaurantData = async () => {
    try {
      const response = await fetch('http://localhost:8080/api/restaurants/yorokobi');
      if (response.ok) {
  ...
  const joinQueue = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8080/api/restaurants/yorokobi/joinQueue', {
        method: 'POST',
        headers: {
  ...
  const startAutoDequeue = () => {
    setInterval(async () => {
      try {
        const response = await fetch('http://localhost:8080/api/restaurants/yorokobi/dequeue', {
          method: 'POST',
        });
